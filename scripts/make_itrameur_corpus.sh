...
do
	...
	content=$(echo "$content" | $SED_CMD -E "s/&/&amp;/g")
	content=$(echo "$content" | $SED_CMD -E "s/</&lt;/g")
	content=$(echo "$content" | $SED_CMD -E "s/>/&gt;/g")
	...
	if [ "$basename" = "en" ]
	then
		content=$(echo "$content" | $SED_CMD -E "s/\"?[Dd]isinformation\"?/disinformation/gI")
		content=$(echo "$content" | $SED_CMD -E "s/\"?[Pp]ropaganda\"?/propaganda/gI")

	# Replace various forms of "Désinformation" with "désinformation", and "Propagande" with "propagande"
	elif [ "$basename" = "fr" ]
	then
		content=$(echo "$content" | $SED_CMD -E "s/\"?[Dd]ésinformation\"?/désinformation/gI")
		content=$(echo "$content" | $SED_CMD -E "s/\"?[Pp]ropagande\"?/propagande/gI")
	fi
